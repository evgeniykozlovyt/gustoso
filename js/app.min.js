...
let slider_dishes = new Swiper('.dishes__container', {
	/*
	effect: 'fade',
	autoplay: {
		delay: 3000,
	...
	*/
	...
	// Arrows
	navigation: {
		nextEl: '.dishes__arrow--next',
		prevEl: '.dishes__arrow--prev',
	},
	on: {
		lazyImageReady: function () {
			ibg();
...
let cakesColumns = document.querySelectorAll('.cakes__column');
let chefNames = document.querySelectorAll('.chef-cakes__name');
let fullScreenContainer = document.querySelector('.full-screen__container');

function resizeCakesColumns() {
	cakesColumns.forEach(element => {
		element.style.height = element.offsetWidth * 250 / 295 + "px";
	});
}

function resizeChefNamesPseudoElements() {
	let maxChefNamesWidth = 0;

	chefNames.forEach(element => {
		let elementWidth = element.querySelector("span").offsetWidth; 

		if (maxChefNamesWidth < elementWidth) {
			maxChefNamesWidth = elementWidth;
		}
	});

	if (maxChefNamesWidth * 1.3 > (document.querySelector('.cakes__chef').offsetWidth - 20) / 2) {
		maxChefNamesWidth = (document.querySelector('.cakes__chef').offsetWidth - 20) / 1.3 / 2
	}
	
	chefNames.forEach(element => {
		element.style.setProperty('--width', maxChefNamesWidth * 1.3 + "px");
	})
}

function resizeFullScreenContainer() {
	fullScreenContainer.style.setProperty('--width', fullScreenContainer.offsetHeight * 939 / 888 + "px");
	fullScreenContainer.style.setProperty('--height', document.querySelector('.full-screen').offsetHeight - 15 + "px");
}

resizeCakesColumns();
resizeChefNamesPseudoElements();
